...
class Doctrine2_Demo extends MX_Controller {
  ...
  public function index() {
    ...
    redirect(base_url() . 'doctrine2_demo/data/');
  }
  ...
  public function data($page = NULL) {
    ...
    // Pagination setup
    $pagination_config['base_url'] = base_url() . 'doctrine2_demo/data/';
    ...
    // Generate table
    // Table headings
    $add_link = base_url() . 'doctrine2_demo/add/';
    $heading = array(
  ...
  public function edit ($id = NULL) {
    ...
    if ($id == NULL && !$this->input->post('save')) {
      redirect(base_url() . 'doctrine2_demo/');
    }
  ...
  public function add () {
    if ($this->input->post('save')) {
      ...
      try {
        ...
        redirect(base_url() . 'doctrine2_demo/data/' . self::$user_page);
      }
      catch (Exception $e) {
        $this->session->set_flashdata('message_error', 'Record could not be saved.');
        redirect(base_url() . 'doctrine2_demo/data/' . self::$user_page);
